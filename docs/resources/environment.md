...
| `edge_id`  | Unique Edge Agent identifier (UUID). Returned by Portainer when the `EnforceEdgeID` setting is enabled, otherwise generated by the provider at create time. Pass it to the Edge Agent as `PORTAINER_EDGE_ID`.          |
